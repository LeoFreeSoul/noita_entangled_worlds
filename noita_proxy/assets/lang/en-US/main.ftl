...
launcher_no_command_3 = You can put `noita_proxy --launch-cmd "%command%"` in steam's launch options to intercept whatever command steam uses to start the game.
...
local_health_spawn_respawn_desc = Each player has separate health. On death, the player returns to the run's starting point while keeping inventory, perks, and gold.
Health-percent-lost-on-reviving = Percent of Max HP lost on reviving
global_hp_loss = Lose HP globally
local_health_alternate_dont_run = Reviving statue does not run
revive_on_drop = Revive player anywhere when dropping heart
...
Local-health-spawn-respawn = Local health (Respawn at start)
...
game_mode_LocalSpawnRespawn = Local health (Respawn at start)
game_mode_PvP = PvP
