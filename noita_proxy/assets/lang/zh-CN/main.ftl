...
local_health_alternate_dont_run = 复活雕像不会跑
revive_on_drop = 丢弃心脏时可在任意位置复活玩家
local_health_desc_1 = 每个玩家都有自己的生命值，所有玩家死亡时本局游戏结束。
local_health_desc_2 = 允许玩家复活
local_health_spawn_respawn_desc = 每个玩家拥有独立生命值；死亡后回到本局初始出生点，并保留法杖、物品、天赋和金币。
...
Local-health-spawn-respawn = 独立生命值模式（死亡回到出生点）
...
game_mode_LocalSpawnRespawn = 独立生命值模式（死亡回到出生点）
game_mode_PvP = PvP
